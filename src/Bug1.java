...
public class Bug1 {
	...
	public static void main(String[] args) throws Exception{
		Thread t1 = new Thread(){
			public void run()
			{
				 s1.removeAll(s2);// should be atomic for s2 too! no any updates to s2 are allowed.
			}
		};
		...
		Thread t2 = new Thread(){
			public void run()
			{
				s2.clear();//any update to S2 causes the AV! So, we have more than one bug in this category.
